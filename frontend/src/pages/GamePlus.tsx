...
const Game: React.FC = () =>
{
	...
	var isUnlockedSubliminal1: boolean = false;
	var isUnlockedRandomRGB2: boolean = false;
	var isUnlockedSubliminal2: boolean = false;
	var isUnlockedFox: boolean = false;
	var isUnlockedSubliminal3: boolean = false;
	...
	function drawBalls(ctx: CanvasRenderingContext2D): void
	{
		...
		// {
			ctx.beginPath();
			ctx.arc(lastGameState.ball.x, lastGameState.ball.y, ballGlobalRadius, 0, Math.PI * 2);
			ctx.fillStyle = "#" + colorCodeRGBBall1;
			ctx.fill();
			ctx.closePath();
			
			if (lastGameState.ball2lock === true)
			{
				ctx.beginPath();
				ctx.arc(lastGameState.ball2.x, lastGameState.ball2.y, ballGlobalRadius, 0, Math.PI * 2);
	...
	function drawScore(ctx: CanvasRenderingContext2D): void
	{
		...
		ctx.fillText(`Score P1: ${lastGameState.score1}`, 10, 20);
		ctx.fillText(`Score P2: ${lastGameState.score2}`, canvasWidth - 100, 20);
	}
	...
	function drawFox(ctx: CanvasRenderingContext2D): void
	{
		if (lastGameState.fox.isUnlocked === true)
		{
			if (lastGameState.fox.isEnraged)
			{
				ctx.drawImage(imageFoxEnraged.current, lastGameState.fox.pos.x, lastGameState.fox.pos.y);
			}
			else if (lastGameState.fox.isEvil === true)
			{
				ctx.drawImage(imageFoxBad.current, lastGameState.fox.pos.x, lastGameState.fox.pos.y);
			}
			else
			{
				ctx.drawImage(imageFoxGood.current, lastGameState.fox.pos.x, lastGameState.fox.pos.y);
			}
		}
	}
	...
	{
		if (lastGameState.score1 + lastGameState.score2 >= 35)
		{
	...
	function difficultyHandler(): void
	{
		...
		{
			difficultyScore = 15;
			isUnlockedRandomRGB1 = false;
			colorCodeRGBBackground = "000000";
			colorCodeRGBScore = "CCCCCC";
			colorCodeRGBBall1 = "CCCCCC";
			colorCodeRGBBall2 = "CCCCCC";
			colorCodeRGBP1 = "CCCCCC";
			colorCodeRGBP2 = "CCCCCC";
			isUnlockedHarkinian = true;
			isUnlockedSubliminal1 = true;
		}
		else if (scoreP1 + scoreP2 === 20 && difficultyScore < 20)
		{
			difficultyScore = 20;
		}
		else if (scoreP1 + scoreP2 === 25 && difficultyScore < 25)
		{
			difficultyScore = 25;
			isUnlockedRandomRGB1 = true;
			isUnlockedRandomRGB2 = true;
			isUnlockedSubliminal2 = true;
		}
		...
		else if (scoreP1 + scoreP2 === 35 && difficultyScore < 35)
		{
			difficultyScore = 35;
			isUnlockedVideo = true;
			isUnlockedSubliminal3 = true;
		}
		...
		else if (scoreP1 + scoreP2 === 45 && difficultyScore < 45)
		{
			difficultyScore = 45;
		}
	...
	function drawSubliminalDepression(ctx: CanvasRenderingContext2D): void
	{
		...
		{
			subliminalSelectorDepression = Math.floor(Math.random() * 100) % (subliminalMessagesArrayDepression.length);
			subliminalDepressionIsActive = true;
		}
	...
	function drawSubliminalSchizophrenia(ctx: CanvasRenderingContext2D): void
	{
		...
		{
			subliminalSelectorSchizophrenia = Math.floor(Math.random() * 100) % (subliminalMessagesArraySchizophrenia.length);
			subliminalSchizophreniaIsActive = true;
		}
	...
	function drawSubliminalHellish(ctx: CanvasRenderingContext2D): void
	{
		...
		{
			subliminalSelectorHellish = Math.floor(Math.random() * 100) % (subliminalMessagesArrayHellish.length);
			subliminalHellishIsActive = true;
		}
	...
	function drawSubliminalMessages(ctx: CanvasRenderingContext2D, timestamp: number): void
	{
		...
		{
			if (timestamp - subliminalStartTime >= 200)
			{
				subliminalDepressionIsActive = false;
				subliminalSchizophreniaIsActive = false;
				subliminalHellishIsActive = false;
			}
		...
		if (canvasRef.current)
		{
			...
			const drawEnhanced = (timestamp: number) =>
			{
				...
				if (elapsed > interval)
				{
					...
					if (isUnlockedSubliminal3 === false)
					{
						drawSubliminalMessages(context, timestamp);
					}
