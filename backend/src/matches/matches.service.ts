...
export class matchesService {
  ...
  async getAll(): Promise<ShowAnyMatchDto[]> {
    ...
    async getAllForUser(user: User): Promise<ShowAnyMatchDto[]> {
        const awayMatches = await this.prisma.match.findMany({ where: { awayPlayerId: user.id } });
        const homeMatches = await this.prisma.match.findMany({ where: { homePlayerId: user.id } });
